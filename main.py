...
#(  _ \( \/ )( ___)/ __)(  _  )/ __)  
# )   / \  /  )__)( (_-. )(_)(( (__   
#(_)\_)  \/  (____)\___/(_____)\___)  
#
# Author : Mahmoud Akram
...
from xml.dom import UserDataHandler, minidom
...
configFile = installDir + "/rvegoc.cfg"
print(installDir)
config = configparser.RawConfigParser()
config.read(configFile)

toolDir = installDir + config.get('rvegoc', 'toolDir')
logDir = installDir + config.get('rvegoc', 'logDir')
yes = config.get('rvegoc', 'yes').split()
...
TermsAndConditions = programcolor.NOTICE + '''
Any illegl activities or attack used by this tool. 
The user will take the full responsibility in ::: action.
Your Choice. Your Decision. Take It Or Leave It. 
Do It Right. Do It Wrong. Consequences Will Be Shown. 
Enjoy!!! 
''' + programcolor.END 

rvegoc = programcolor.NOTICE + '''
All the new and old hacking tools will be available in RVEGOC.
You have the freedom to choose whatever tool you like. 
'''
...
def rev_conditions():
    while not config.getboolean("rvegoc", "rev_conditions"):
        clearscript()
        print(TermsAndConditions)
        print(rvegoc)
        agree = input("Agree to the terms and conditions (Y/N) ").lower()
        if agree in yes:
            config.set('rvegoc', 'rev_conditions', 'true')
...
                                                       
    ...
    ''' + programcolor.END + '''
     ...
     ''')
        ...
        if pchoice == "1":
            ...
            sys.exit()
        elif pchoice == "\r" or pchoice == "\n" or pchoice == "" or pchoice == " ":
            self.__init__()
        else:
            try:
                print(os.system(pchoice))
            except:
                pass
        self.completed()
    ...
    def update(self):
        os.system("git clone --depth=1 https://github.com/ReTnUh-J/rvegoc.git")
        os.system("cd rvegoc && bash ./update.sh")
        os.system("rvegoc")


class SniffingandSpoofing:
    menuLogo = '''
   __       _  __  __ _             
...
class PostExploit:
    menuLogo = '''
     ___          _     __            _       _ _   
    / _ \___  ___| |_  /__\_  ___ __ | | ___ (_) |_ 
   / /_)/ _ \/ __| __|/_\ \ \/ / '_ \| |/ _ \| | __|
  / ___/ (_) \__ \ |_//__  >  <| |_) | | (_) | | |_ 
  \/    \___/|___/\__\__/ /_/\_\ .__/|_|\___/|_|\__|
                               |_|                  
    '''
...
class Reconnaissance:
    menuLogo = '''
    _____        __         ___      _   _               
    \_   \_ __  / _| ___   / _ \__ _| |_| |__   ___ _ __ 
     / /\/ '_ \| |_ / _ \ / /_\/ _` | __| '_ \ / _ \ '__|
  /\/ /_ | | | |  _| (_) / /_\\ (_| | |_| | | |  __/ |   
  \____/ |_| |_|_|  \___/\____/\__,_|\__|_| |_|\___|_|   
    ...
    def menu(self, target):
        ...
        try:
            if response == "1":
                ...
                response = input(rvegoc_pro_continue)
            elif response == "2":
...
class setoolkit:
    def __init__(self):
        ...
        if not self.installed():
            ...
            print(already_installed_system)
            self.run()
        response = input(rvegoc_pro_continue)
...
class XSStrike:
    XSStrikeLogo = '''
  __  ____  __       __  _____  __   _____        __ 
  \ \/ / _\/ _\     / _\/__   \/__\  \_   \/\ /\ /__\
   \  /\ \ \ \ _____\ \   / /\/ \//   / /\/ //_//_\  
   /  \_\ \_\ \_____|\ \ / / / _  \/\/ /_/ __ \//__  
...
class PasswordAttacks:
    menuLogo = '''
    ___  _    __  __         _   _____  _____  _      ___       
    / _ \/_\  / _\/ _\       /_\ /__   \/__   \/_\    / __\ /\ /\
   / /_)//_\\ \ \ \ \ _____ //_\\  / /\/  / /\//_\\  / /   / //_/
  / ___/  _  \_\ \_\ \_____/  _  \/ /    / / /  _  \/ /___/ __ \ 
...
class WirelessTesting:
    menuLogo = '''
   __    __ _____  __    __  __    __  __  __         _   _____  _____ 
  / / /\ \ \\_   \/__\  /__\/ /   /__\/ _\/ _\       /_\ /__   \/__   \
  \ \/  \/ / / /\/ \// /_\ / /   /_\  \ \ \ \ _____ //_\\  / /\/  / /\/
   \  /\  /\/ /_/ _  \//__/ /___//__  _\ \_\ \_____/  _  \/ /    / /   
...
class ExploitationTools:
    menuLogo = '''
     ____  __  ___  __    ___ _____  _____ 
    /__\ \/ / / _ \/ /   /___\\_   \/__   \
   /_\  \  / / /_)/ /   //  // / /\/  / /\/
  //__  /  \/ ___/ /___/ \_//\/ /_   / /   
...
def commix():
    ...
    if YesORNo():
        ...
        Reconnaissance.completed("Commix")
...
def inurl():
    ...
    WebsiteAttacks.completed("InurlBR")
...
def wppluginscan():
    ...
    def scan(site, dir):
        global resp
        try:
            conn = httplib2.HttpLib2ErrorWithResponse.HTTPConnection(site)
...
def sqlmap():
    ...
    if YesORNo():
        ...
        Reconnaissance.completed("SQLMap")
...
def grabuploadedlink(url):
    try:
        ...
        for dir in directories:
            currentcode = urlopen(url + dir).getcode()
...
def grabshell(url):
    try:
        ...
        for upl in upload:
            for shell in shells:
                currentcode = urlopen(upl + shell).getcode()
                if currentcode == 200:
                    print ("-------------------------")
                    print ("  [ ! ] Found Shell:  " + \
                        str(upl + shell) + " [ ! ]")
...
def ssls():
    ...
    if YesORNo():
        ...
        SniffingandSpoofing.completed("SSlStrip")
...
def check_gravityforms(sites):
    ...
    for site in sites:
        try:
            if urlopen(site + 'wp-content/plugins/gravityforms/gravityforms.php').getcode() == 403:
    ...
    def findPanels(self):
        ...
        for site in self.sites:
            for admin in adminList:
                try:
                    if urlopen(site + admin).getcode() == 200:
 ...
 # find ZIP files
    ...
    def findZip(self):
        ...
        for site in self.sites:
            for zip1 in zipList:
                try:
                    if urlopen(site + zip1).getcode() == 200:
    ...
    def findUp(self):
        ...
        for site in self.sites:
            for up in upList:
                try:
                    if (urlopen(site + up).getcode() == 200):
                        html = urlopen(site + up).readlines()
                        for line in html:
...
def grabsqli(ip):
    try:
        print (programcolor.OKBLUE + "Check_Uplaod... ")
        print ('\n')
...
def githubContributors(self):
  ...
  ░                                                        ░ 
     ''')
        contributorsURL = 'https://api.github.com/repos/ReTnUh-J/rvegoc/contributors'
